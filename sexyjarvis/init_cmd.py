"""Generate a SEXYJARVIS.md from a workspace scan (like Claude Code /init).

Cheap heuristic scan — no LLM call — so it's instant. The user can edit the
output afterwards. We detect common project types and surface entry points.
"""
...
# (filename, label) — first hit wins per category.
_STACK_HINTS = [
...
def _detect_commands(workspace: Path) -> list[str]:
    """Pull a few likely dev commands from package.json or pyproject."""
    cmds: list[str] = []
...
def generate_sexyjarvis_md(workspace: Path, overwrite: bool = False) -> tuple[Path, bool, str]:
    """Write SEXYJARVIS.md. Returns (path, created_bool, status_message)."""
    target = workspace / "SEXYJARVIS.md"
    ...
    lines = [
        f"# {workspace.name}",
        "",
        "_Standing instructions for SexyJarvis. Edit freely — this file is "
