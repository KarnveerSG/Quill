...
def export_markdown(session, target: Path) -> Path:
    ...
    lines: list[str] = [f"# SexyJarvis session — {time.strftime('%Y-%m-%d %H:%M:%S')}", ""]
...
def discover_custom_commands(workspace: Path) -> dict[str, CustomCommand]:
    """Find .sexyjarvis/commands/*.md (and ~/.sexyjarvis/commands/) as user commands."""
    out: dict[str, CustomCommand] = {}
    for root in (workspace / ".sexyjarvis" / "commands", Path.home() / ".sexyjarvis" / "commands"):
        if not root.is_dir():
            continue
